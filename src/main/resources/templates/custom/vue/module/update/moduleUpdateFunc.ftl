...
const _${tableInfo.tableNameHump}Tree = async () => {
    ...
    if (res.ok) {
        ...
        <#--上传头像、上传图片成功回调-->
            <#if field.uiType == "PROFILE_PICTURE_UPLOAD" || field.uiType == "PICTURE_UPLOAD">
const handle${field.columnNameBigHump}UploadSuccess = (response: any) => {
    _data.value.${field.columnNameHump} = response.data
}
            </#if>
        </#if>
    </#if>
