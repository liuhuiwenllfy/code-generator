...
import VeBatchUploadPicture from 've-upload/ve-batch-upload-picture/index.vue'
<#break>
</#if>
</#list>
...
import VeBatchUploadFile from 've-upload/ve-batch-upload-file/index.vue'
<#break>
</#if>
</#list>
...
const ${tableField.columnNameHump}Tree = async () => {
  // TODO 查询树接口
  const res = <resData>await get${tableField.columnNameBigHump}Tree({showTopNode: true})
  if (res.ok) {
...
<#--                    单选框组-->
                ...
                <el-radio-group v-model="_data.${tableField.columnNameHump}">
                    <el-radio v-for="(item, index) in _${tableField.columnNameHump}Ref" :key="index" :label="item.key">{{item.value}}</el-radio>
                </el-radio-group>
<#--                    多选框组-->
                ...
                <el-checkbox-group v-model="_data.${tableField.columnNameHump}">
                    <el-checkbox v-for="(item, index) in _${tableField.columnNameHump}Ref" :key="index" :label="item.key">{{item.value}}</el-checkbox>
                </el-checkbox-group>
<#--                    树形选择-->
